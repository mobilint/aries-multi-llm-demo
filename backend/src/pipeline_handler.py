...
class LLMHandler:
    ...
    def _prefill_cache(self):
        ...
        self.initial_cache: Cache = self.model._get_cache("eager", 1, 4096)
        ...
        if len(self.base_conversation) > 0:
            ...
            if isinstance(self.initial_cache, MobilintCache):
                output = self.model.generate(
                    **inputs,
                    max_new_tokens=1,
                    use_cache=True,
                    past_key_values=self.initial_cache,
            ...
            self.initial_past_token_ids = output.detach().cpu().numpy()
        ...
        logging.info(f"[aries{self.dev_no} {self.target_cores[0]}] Prefill completed in {time() - start:.2f} sec, seq_length: {self.initial_cache.get_seq_length()}")
        
    def reset_cache(self):
        logging.info(f"[aries{self.dev_no} {self.target_cores[0]}] Reset cache for model: {self.model_id}, seq_length: {self.initial_cache.get_seq_length()}")
        self._load_txt_files()
        ...
        try:
            ...
            def generation_wrapper(**kwargs):
                try:
                    ...
                    handler.past_token_ids = output.detach().cpu().numpy()
