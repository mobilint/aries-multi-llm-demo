...
DEMO_NAME="ARIES Multi LLM Demo"
DEMO_DIR_NAME="aries-multi-llm-demo"
...
models = json.loads(Path("src/models.json").read_text(encoding="utf-8"))
downloads = []
seen = set()
...
for model in models:
    ...
    repo_id = re.sub(r"^[^/]+", "mobilint", model_id, count=1)
...
if [ -f /usr/share/applications/multi-llm-demo.desktop ]; then
  sudo rm /usr/share/applications/multi-llm-demo.desktop
fi
